...
def main(argv=None):
    ...
    assert len(
        [a for a in multi_args if a == True]) < 2, 'At most one of the "-f", "-l" and "-s" arguments can be specified.'
...
def argparser():
    ...
    ap.add_argument('-f', '--first-type', default=False, action='store_true',
                    help='Use first type in for BI+ sequences with multiple types.')
    ap.add_argument('-l', '--last-type', default=False, action='store_true',
                    help='Use first type in for BI+ sequences with multiple types (default).')
    ap.add_argument('-s', '--split-multi', default=False, action='store_true',
