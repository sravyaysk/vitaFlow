...
class VfModel(base.Layer):
  ...
  `VfModel` has three typical usages:
  ...
  2. Layer: The method `call` enables `VfModel` to be used a callable by
     ...
     * `loss`, which takes the logits, forms any missing training loss, and sums
       all loss terms.
  3. Inference: The method `infer` enables `VfModel` to make sequence
  ...
  def model_fn_sharded(self, sharded_features):
    ...
    if self.use_body_sharded():
      ...
      if "training" in losses:
        log_info("Skipping VfModel top and loss because training loss "
                 "returned from body")
  ...
  def bottom(self, features):
    ...
    if not self._problem_hparams:
      log_warn("Without a Problem, VfModel.bottom is a passthrough.")
      return features
  ...
  def body(self, features):
    ...
    Most `VfModel` subclasses will override this method.
  ...
  def _top_single(self, body_output, target_modality, features):
    if not target_modality:
      log_warn("Without a Problem, VfModel.top is a passthrough.")
      return body_output
  ...
  def set_mode(self, mode):
    ...
    log_info("Setting VfModel mode to '%s'", mode)
    ...
    if self._problem_hparams:
      ...
      #since modality is now HParams we need to get it as dict explicitly
      for modality_obj in six.itervalues(self._problem_hparams.modality.todict()):
