...
def create_naive_receipt(file_path):
    ...
    draw = insert_text(draw=draw, x=20, y=80, text="Invoice : " + str(get_random_number(min_num_chars=4, max_num_chars=6)), font_size=10)
...
def train():
    number_files = 400
    os.makedirs(OUT_DIR + "/train/images/")
    os.makedirs(OUT_DIR + "/train/annotations/")

    for i in tqdm(range(number_files)):
        create_naive_receipt(OUT_DIR +"/train/images/"+str(i)+".png")
        replicate_xml(out_file_path=OUT_DIR +"/train/annotations/"+str(i)+".xml")

def test():
    number_files = 40
    os.makedirs(OUT_DIR + "/test/images/")
    os.makedirs(OUT_DIR + "/test/annotations/")

    for i in tqdm(range(number_files)):
        create_naive_receipt(OUT_DIR +"/test/images/"+str(i)+".png")
        replicate_xml(out_file_path=OUT_DIR +"/test/annotations/"+str(i)+".xml")

def eval():
    number_files = 10
    os.makedirs(OUT_DIR + "/eval/images/")
    os.makedirs(OUT_DIR + "/eval/annotations/")

    for i in tqdm(range(number_files)):
        create_naive_receipt(OUT_DIR +"/eval/images/"+str(i)+".png")
        replicate_xml(out_file_path=OUT_DIR +"/eval/annotations/"+str(i)+".xml")
